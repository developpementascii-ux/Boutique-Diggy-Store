  ...
  const periodSummary = useMemo(() => {
    ...
    (sales || []).forEach((s) => {
      if (s.date && activeDateKeys.has(getLocalDateKey(s.date))) {
        if (Number(s.remainingCredit) > 0) {
          totalGrantedCredit += Number(s.remainingCredit);
    ...
    (repairs || []).forEach((r) => {
      if (r.createdAt && activeDateKeys.has(getLocalDateKey(r.createdAt)) && r.status !== 'delivered') {
        if (Number(r.remainingDue) > 0) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div className="glass-kpi-grid">
        ...
        {/* KPI 4: Bénéfice Net Réel (+/-) */}
        <div
          className="glass-kpi-card"
          style={{
            borderColor: periodSummary.netProfit >= 0 ? 'rgba(16, 185, 129, 0.3)' : 'rgba(239, 68, 68, 0.3)',
          }}
        >
          <div
            className="glass-kpi-glow"
            style={{ background: periodSummary.netProfit >= 0 ? '#10b981' : '#ef4444' }}
          />
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
            <div>
              <span
                style={{
                  fontSize: '0.75rem',
                  fontWeight: 700,
                  color: periodSummary.netProfit >= 0 ? '#10b981' : '#ef4444',
            ...
            <div
              style={{
                ...
                height: '32px',
                borderRadius: '8px',
                background: periodSummary.netProfit >= 0 ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                color: periodSummary.netProfit >= 0 ? '#34d399' : '#f87171',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
              }}
          ...
          <div
            style={{
              background: periodSummary.netProfit >= 0 ? 'rgba(16, 185, 129, 0.08)' : 'rgba(239, 68, 68, 0.08)',
              padding: '0.35rem 0.5rem',
              borderRadius: '8px',
              border: `1px solid ${periodSummary.netProfit >= 0 ? 'rgba(16, 185, 129, 0.18)' : 'rgba(239, 68, 68, 0.18)'}`,
              marginBottom: '0.5rem',
            }}
          >
            <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '3px' }}>
              <span
                style={{
                  ...
                  height: '6px',
                  borderRadius: '50%',
                  background: periodSummary.netProfit >= 0 ? '#34d399' : '#f87171',
            ...
            >
              {periodSummary.netProfit >= 0 ? '+' : ''}{formatMoney(periodSummary.netProfit)}
            </div>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.72rem', color: 'var(--text-muted)' }}>
            <span>Marge Nette : <strong>{periodSummary.netProfitMargin}%</strong></span>
            <span style={{ color: periodSummary.netProfit >= 0 ? '#34d399' : '#f87171', fontWeight: 700 }}>
              {periodSummary.netProfit >= 0 ? `✓ ${periodSummary.profitableDaysCount}j gagnants` : `⚠️ ${periodSummary.lossDaysCount}j déficitaires`}
            </span>
          </div>
        </div>
